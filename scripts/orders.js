...
import { getPaintColors, getInteriors, getTechnologies, getWheels, getOrders, getCatagories } from "./database.js"
...
const buildOrderListItem = (order) => {
    ...
    const pickedCatagory = catagories.find(
        (catagory) => {
            if (catagory.id === order.catagoryId) {
                return true
            }
        }
    )
        //adding all prices togeter in a variable of total price
    let totalPrice = (foundColor.price + foundInterior.price + foundTechnology.price + foundWheel.price)
    //Make an if else statement for * the total cost to the catagory selected
        if (pickedCatagory.id === 1) {
            totalPrice 
        }
        else if (pickedCatagory.id === 2) {
            totalPrice *= 1.5
        }
        else if (pickedCatagory.id === 3) {
            totalPrice *= 2.25
        }
